<script lang="ts">
	...
	import { selfCheck } from '$lib/utils/utils';
	export let link: Link;
	let selected: boolean = false;
	import Icon from '@iconify/svelte';
	import { page } from '$app/stores';
	const icon = `heroicons:${link.icon}`;
	$: selected = selfCheck($page.url.pathname, link.dest);
</script>
...
{#if selected}
	<li>
		...
		<a
			href={link.dest}
			class="group flex gap-x-3 rounded-md bg-gray-50 p-2 text-sm font-semibold leading-6 text-indigo-600 items-center"
		>
